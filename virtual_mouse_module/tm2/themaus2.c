...
static int __init vmaus_init(void)
{
	...
	register_chrdev(239, "mausv2", &ms_file_operations);
	vmaus_dev = input_allocate_device();

	vmaus_dev->name = "Alfonso's virtual mouse";
	vmaus_dev->phys = "rpcmouse/input1";
	vmaus_dev->id.bustype = BUS_HOST;
	vmaus_dev->id.vendor  = 0x0005;
	vmaus_dev->id.product = 0x0001;
	vmaus_dev->id.version = 0x0100;
